...
extern uint8_t BMS_resetFlag;


static uint16_t **owPup;
static uint16_t **owPdn;


// For external access
uint8_t *HV_slavesNumber = &slavesNumber;
cell_asic *HV_slaves;
BMSmodule *HV_modules;
uint8_t *HV_fail_reason = &fail_reason;
uint8_t *HV_prot_state = &prot_state;
uint8_t *HV_protRet = &protRet;
...
void init_state() {
    ...
    bmsconf.slavesCount = 8;
    bmsconf.OVPT = 42000;
    bmsconf.UVPT = 25000;
    bmsconf.current_offset = 30080;
    bmsconf.OT_th = 6500; // 60C
    bmsconf.UT_th = 23000; // 0C

    // OCP configuration
    bmsconf.OCP_d_th = -10000; // Voor HV: x100 (100A)
    bmsconf.OCP_c_th = 1000;
    ...
    owPdn = malloc(bmsconf.slavesCount * sizeof(uint16_t*));
    owPup = malloc(bmsconf.slavesCount * sizeof(uint16_t*));

    for(uint8_t i = 0; i < bmsconf.slavesCount; i++) {
    	owPdn[i] = malloc(18*sizeof(uint16_t));
    	owPup[i] = malloc(18*sizeof(uint16_t));
    }
    ...
    for (uint8_t i = 0; i < bmsconf.slavesCount; i++) {
		modules[i].cellNumber = 16;
		modules[i].tempNumber = 16;
    }
    ...
    ADBMS1818_adcvax(MD_422HZ_1KHZ, 0);
    (void)ADBMS1818_pollAdc();
...
void wait_state() {
	...
	if (counted_time[TO_timer] > 100) { // 17ms * 1.5 + marge
...
static uint16_t somCRCfouten = 0;
...
void read_start_state() {
	...
	if(cv_counter >= 100) {
		cv_counter = 0;
//		ADBMS181x_rdcfg(slavesNumber, slaves);
...
//	    ADBMS1818_wrcfgb(slavesNumber, slaves);
    ...
    if(!(pec_fail & 0b100)) {
		...
		// OVP UVP protection
		protRet = checkOvpUvp(slavesNumber, slaves, modules);
		...
		if(protRet) { // over/under
			...
			} else if (counted_time[OVP_timer] > 750) { // Timer already running. Ran out?
				// 500ms passed; Fault;
				hvRelayOpen();
...
//    	modules[1].temperature[1] = 15000;
    ...
    // 600V check
	if(cv_counter == 0) { // all cell voltages were read
		if(!(pec_fail & 0b1)) {
			uint32_t sum = 0;
			for(uint8_t i = 0; i < slavesNumber; i++) {
				for(uint8_t cell = 0; cell < modules[i].cellNumber; cell++) {
					sum += slaves[i].cells.c_codes[cell];
				}
			}
	...
	if((!(prot_state & 0b00001111)) && (BMS_resetFlag == 1)) { // There was a latched error, but currently no errors
			prot_state = 0;
			hvRelayRelease();
			BMS_resetFlag = 0;
	}
...
static uint8_t owConversionCount = 0;
uint8_t HV_owFail = 0;


void OW_conv_state() {
	if (owConversionCount < 10) {
		owConversionCount++;
		ADBMS181x_adow(MD_7KHZ_3KHZ, 1, CELL_CH_ALL, 0); // Snelle conv. Pull up, Geen dis
		resetCounter(TO_timer); // Reset timeout timer
	    BMSstate = state_OW_wait_state;
	    return;

	} else if (owConversionCount < 20) {
		owConversionCount++;
		ADBMS181x_adow(MD_7KHZ_3KHZ, 0, CELL_CH_ALL, 0); // Snelle conv. Pull down, Geen dis
	    resetCounter(TO_timer); // Reset timeout timer
	    BMSstate = state_OW_wait_state;
	    return;

	} else if (owConversionCount == 20) { // metingen klaar

		// cell delta = PUP - DN
		int16_t cell_delta[18];

		for (uint8_t i = 0; i < slavesNumber; i++) { // loop modules
			for(uint8_t j = 0; j < modules[i].cellNumber; j++) { // loop cells
				cell_delta[j] = owPup[i][j] - owPdn[i][j];
			}

//			if (owPup[i][0] == 0) { // c0 open
//				HV_owFail++;
//			}
//			if (owPdn[i][17] == 0) { // c18 open
//				HV_owFail++;
//			}
			for (uint8_t k = 0; k < 18; k++) {
				if (cell_delta[k] < -4000) { //c k open
					HV_owFail++;
				}
			}
		}
...
void OW_wait_state() {
	...
	if(counted_time[TO_timer] > 15){
		...
		while(ADBMS1818_rdcv(0, slavesNumber, slaves)) {
			readTries++;
			if(readTries > 5) {
				BMSstate = state_fail;
				fail_reason = 10;
		...
		if (owConversionCount == 10) { // tweede keer, pup
			for (uint8_t i = 0; i < slavesNumber; i++) { // loop modules
				for(uint8_t j = 0; j < modules[i].cellNumber; j++) { // loop cells
					owPup[i][j] = slaves[i].cells.c_codes[j];
				}
			}
		} else if (owConversionCount == 20) { // tweede keer, pdn
			for (uint8_t i = 0; i < slavesNumber; i++) { // loop modules
				for(uint8_t j = 0; j < modules[i].cellNumber; j++) { // loop cells
					owPdn[i][j] = slaves[i].cells.c_codes[j];
				}
			}
		}
