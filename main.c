...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Contains code for the handling of the FSM for the Powertrain Controller
  * 				 (PTC).
  ******************************************************************************
  * @attention
  *
...
int main(void)
{
  ...
  MX_COMP1_Init();
  /* USER CODE BEGIN 2 */
  PTC_FDCAN_Setup(&hfdcan1);
  General_FDCAN_Setup(&hfdcan2);
//  HAL_GPIO_WritePin(GPIOG, (uint16_t)1<<1, GPIO_PIN_SET);
...
//  IVT_SetConfiguration();
  ...
  {
//	  BMS_CS = 0;
//	  (void)LV_BMS_FSM_update();
	  BMS_CS = 1;
	  (void)HV_BMS_FSM_update();
//	  hvRelayRelease(); // TODO temp
	  PTC_UpdateFSM();
    /* USER CODE END WHILE */
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1|GPIO_PIN_15|GPIO_PIN_8|GPIO_PIN_9, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : PB1 PB15 PB8 PB9 */
  GPIO_InitStruct.Pin = GPIO_PIN_1|GPIO_PIN_15|GPIO_PIN_8|GPIO_PIN_9;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
...
{
	// Precharge time (95%): 3.8 s --> Timer Interrupt at 7.6 s
	if (htim->Instance == TIM6)  // Check if interrupt is from TIM6
	{
		if (currentState == PreCharge_State) // Only act if we're still precharging
		{
			RaiseError(ERROR_PRECHARGE_FAILURE); // Raise Precharge timer failure error
			currentState = Fail_State;       // Go to Fail state
		}
		DisableTimer(&htim6);
	}
...
/* Redirect printf output to USART */
